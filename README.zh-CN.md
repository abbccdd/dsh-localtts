...
[English](README.md) | 简体中文 · **v0.1.7 发布候选版** · MIT
...
2. **项目文件夹**：粘贴已有安装的完整目录，离开输入框时自动查找，也可点击 **自动查找文件**。支持项目根目录，以及常见的外层整合包目录。IndexTTS 与 GPT-SoVITS 分别保存自己的项目、模型和角色配置；切换回来会恢复之前填写的路径，不会把另一引擎的路径带过来。
