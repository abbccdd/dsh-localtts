...
// (GET /dsh-tts-api/notify?s=N) to read the alert aloud. Dedup by approval
// id so one request is never announced twice, even across replays.
// ---------------------------------------------------------------------------
const notifyState = {
  queue: [],       // [{ seq, kind: 'approval'|'approval-decided', id, toolName, reason, outcome }]
  seen: new Set(), // approval ids already ingested (dedup)
  seq: 0,
  cap: 20,
  seenCap: 200
};
...
function ingestSessionEvent(event) {
  ...
  if (type === 'approval/asked') {
    ...
    if (!id || notifyState.seen.has(id)) return;
    notifyState.seen.add(id);
    notifyState.queue.push({
      seq: ++notifyState.seq,
      kind: 'approval',
      id,
      toolName: truncateForSpeech(event.toolName, 40),
      reason: truncateForSpeech(event.reason, 80)
    });
    ...
    notifyState.queue.push({
      ...
      id,
      outcome: ['granted', 'rejected'].includes(outcome) ? outcome : 'settled'
    });
  } else {
    return;
  }
