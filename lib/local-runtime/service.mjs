...
import { SentenceBuffer, manualSentences } from './sentence-buffer.mjs';
...
export class LocalRuntimeService {
  ...
  configure({ clientId, config, sessionId = '', autoRead = false }) {
    ...
    if (c.autoRead && c.config.mode === 'process') this.prepare(c);
    return { ok: true };
  }
  prepare(c) {
    if (c.preparing || c.provider) return;
    const provider = c.provider = this.pool.acquire(c.config);
    const task = Promise.resolve().then(() => provider.healthCheck()).catch(() => {
      if (c.provider === provider) this.release(c);
    });
    c.preparing = task;
    task.finally(() => { if (c.preparing === task) c.preparing = null; });
  }
  client(id) {
  ...
  release(c) { c.provider?.dispose(); c.provider = null; c.preparing = null; }
  add(c, j, parts) {
  ...
  read(c, text) {
    ...
    this.stop(c);
    const j = this.newJob(c, 'manual'); j.done = true; this.add(c, j, manualSentences(text));
    return { jobId: j.id };
  }
  stop(c, source) {
